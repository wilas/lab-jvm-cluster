...
Vagrant::Config.run do |config|
  ...
  config.vm.define :marlin01 do |app_config|  
    ...
    app_config.vm.network :hostonly, "77.77.77.161"
    app_config.vm.share_folder "v-root", "/vagrant", "."
    app_config.vm.forward_port 8080, 6881
  ...
  config.vm.define :marlin02 do |app_config|  
    vm_name= "marlin02"
    app_config.vm.box = "SL64_box"
    app_config.vm.host_name = "#{vm_name}.farm"
    app_config.vm.customize ["modifyvm", :id, "--memory", "512", "--name", "#{vm_name}"]
  
    app_config.vm.network :hostonly, "77.77.77.162"
    app_config.vm.share_folder "v-root", "/vagrant", "."
    app_config.vm.forward_port 8080, 6882
  ...
  config.vm.define :canoe01 do |web_config|  
    ...
    web_config.vm.network :hostonly, "77.77.77.171"
    web_config.vm.share_folder "v-root", "/vagrant", "."
    web_config.vm.forward_port 80, 7081
